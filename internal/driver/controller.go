...
func (c *controllerServer) ControllerExpandVolume(ctx context.Context, req *csi.ControllerExpandVolumeRequest) (*csi.ControllerExpandVolumeResponse, error) {
	...
	if err != nil {
		// Block volumes cannot be resized while they are attached to an instance.
		if vol.ContentType == "block" && strings.TrimSpace(err.Error()) == "In use" {
			return nil, status.Errorf(codes.FailedPrecondition, "ExpandVolume: Block volume %q cannot be resized while attached to an instance.", volName)
		}
